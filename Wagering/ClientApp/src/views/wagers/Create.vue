    ...
    <v-container>
      <v-data-table :headers="headers" :items="wager.hosts" hide-default-footer>
        ...
        <template v-slot:item.receivablePt="{ item }">
          <v-progress-circular :value="item.receivablePt" class="overline">
            {{
            item.receivablePt
            }}
          </v-progress-circular>
        ...
        <template v-slot:item.payablePt="{ item }">
          <v-progress-circular :value="item.payablePt" class="overline">
            {{
            item.payablePt
            }}
          </v-progress-circular>
      ...
      this.$axios
        ...
        .then((response: AxiosResponse) => {
          this.$microsoft.signalr.invoke(
            "AddToGroup",
            response.data.wager.groupName
          );
          const users = response.data.wager.hosts
          ...
          this.$router.push({
            name: pathName,
            params: { id: response.data.wager.id }
          } as RawLocation);
        });
    }
  }
